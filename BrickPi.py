...
class EV3ColorSensor(Sensor):
    ...
    def get_color_name(self) -> str:
        ...
        This function should be used when the sensor is in 'id' mode.
        """
        if self.mode != self.Mode.ID:
            self.set_mode(self.Mode.ID)
            self.wait_ready()

        color_id = int(self.get_value())  # Assuming get_value returns a color ID
        return self.COLOR_NAMES.get(color_id, "Unknown")
